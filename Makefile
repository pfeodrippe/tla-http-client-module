...
build:
	...
	$(clj_cmd) -X:depstar uberjar :jar target/tla-http-client-module.jar :sync-pom true :version '"0.2.0"' :group-id '"io.github.pfeodrippe"' :artifact-id '"tladeps-http-client-module"' :exclude '["clojure/.*", "com/.*", "javax/.*", "org/.*", "pcal/.*", "tla2sany/.*", "tla2tex/.*", "util/.*", "kaocha/.*", "CommonsMath.*", "META_INF/.*", "dynapath/.*", "babashka/.*", "tlc2/util/.*", "tlc2/tool/.*", "tlc2/value/.*", "tlc2/debug/.*", "tlc2/model/.*", "tlc2/output/.*", "tlc2/module/.*", "tlc2/pprint/.*", "cheshire/.*", "clj_http/.*", "slingshot/.*", "riddley/.*", "potemkin/.*"]'
